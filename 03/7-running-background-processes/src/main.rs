#![allow(renamed_and_removed_lints, clippy::vec_init_then_push)]
...
fn main() {
  ...
  let c1 = CounterFuture { count: 0 };
  let c2 = CounterFuture { count: 0 };

  let t1 = spawn_task!(c1, FutureType::High);
  let t2 = spawn_task!(c2);
  let t3 = spawn_task!(async_fn());
  let t4 = spawn_task!(
    async {
      async_fn().await;
      async_fn().await;
